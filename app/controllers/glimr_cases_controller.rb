class GlimrCasesController < ApplicationController
  ...
  def send_emails
    ...
    NotifyMailer.application_details_copy(current_tribunal_case, :taxpayer, case_to_string).deliver_later if current_tribunal_case.send_taxpayer_copy?
    NotifyMailer.application_details_copy(current_tribunal_case, :representative, case_to_string).deliver_later if current_tribunal_case.send_representative_copy?
  end
