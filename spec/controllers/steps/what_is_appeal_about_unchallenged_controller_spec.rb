...
RSpec.describe Steps::WhatIsAppealAboutUnchallengedController, type: :controller do
  ...
  describe '#update' do
    let(:what_is_appeal_about_form) { instance_double(WhatIsAppealAboutForm) }

    before do
      expect(WhatIsAppealAboutForm).to receive(:new).and_return(what_is_appeal_about_form)
    end

    context 'when the form saves successfully' do
      before do
        expect(what_is_appeal_about_form).to receive(:save).and_return(true)
      end
    ...
    context 'when the form fails to save' do
      before do
        expect(what_is_appeal_about_form).to receive(:save).and_return(false)
      end
