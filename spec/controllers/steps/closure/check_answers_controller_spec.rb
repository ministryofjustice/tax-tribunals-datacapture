...
RSpec.describe Steps::Closure::CheckAnswersController, type: :controller do
  ...
  context 'HTML format' do
    it 'assigns the presenter' do
      expect(CheckAnswers::ClosureAnswersPresenter).to receive(:new).with(tribunal_case, format: :html).and_return(presenter)
  ...
  context 'PDF format' do
    ...
    it 'generates and sends the case details PDF' do
      expect(CheckAnswers::ClosureAnswersPresenter).to receive(:new).with(tribunal_case, format: :pdf).and_return(presenter)
